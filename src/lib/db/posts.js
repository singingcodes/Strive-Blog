...
import { getPosts } from "../fsTools.js";
import uniqid from uniqid

export const saveNewPost = async newPostsData => {
    const newPost = {...newPostsData, createdAt: new Date(), id: uniqid(), comments: []}
...
export const findPostByIdAndDelete = async postId => {
    ...
    if (posts.length === remainingPosts.length) throw createError(404, `post with id ${postId} not found`)
